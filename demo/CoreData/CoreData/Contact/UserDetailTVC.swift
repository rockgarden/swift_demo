...
class EditTVC: UITableViewController {
    ...
    @IBAction func saveBtnClick(_ sender: Any) {
        let user = User(avatarImg: UIImagePNGRepresentation(UIImage(named: "avatar1")!)!, isRelative: relativeSwitch.isOn ? true : false, name: nameTF.text ?? "", updateTime: Date(), viewTimes: 1, mobile: mobileTF.text ?? "")

        UserVC.insertData(contactInfo: user)
