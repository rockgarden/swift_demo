...
class SceneViewController : UIViewController, SCNSceneRendererDelegate {
    ...
    private func showData() {
        ...
        chartNode.rotation = SCNVector4(x: 1, y: 0, z: 0, w: Float(-Double.pi/2))
    }
    ...
    private func createChart(_ type:ChartType) -> SCNNode {
        ...
        for idx in 0 ..< numColumns {
            ...
            let posY = min.y - 2.0
            txtNode.position = SCNVector3(x: posX, y: posY, z: 0.0)
            txtNode.rotation = SCNVector4(x: 0, y: 0, z: 1, w: Float(-Double.pi/2))
            node.addChildNode(txtNode)
        }
        ...
        for i in 0 ..< numRows {
            for j in 0 ..< numColumns {
                ...
                aNode.rotation = SCNVector4(x: 1, y: 0, z: 0, w: Float(-Double.pi/2))
