...
open class ImagePickerAction {
    ...
    public convenience init(title: String, rawSecondaryTitle: String? = nil, style: ImagePickerActionStyle = .default, handler: @escaping Handler, secondaryHandler: SecondaryHandler? = nil) {
        ...
        //self.init(title: title, secondaryTitle: secondaryTitle.map { $0 }, style: style, handler: handler, secondaryHandler: secondaryHandler) //secondaryTitle.map { string in { _ in string }}
    }
