...
extension DetailViewController {
    override func targetViewController(forAction action: Selector, sender: Any?) -> UIViewController? {
        print("detail view controller target for \(action) \(String(describing: sender))...")
        let result = super.targetViewController(forAction: action, sender: sender)
        print("detail view controller target for \(action), returning \(String(describing: result))")
        return result
    }
