...
open class LoadingButton: UIButton {
    ...
    fileprivate func setUp() {
        stateLayer = StateLayer(frame: self.bounds,delegate: self)
    ...
    open func addScuessPresentVC(_ toVC:UIViewController) {
        ...
        self.scuessTransition =  ZoomTransition(duration: 0.6)
        self.toVC = toVC
    }
    ...
    fileprivate func getBottomConstraint () -> NSLayoutConstraint? {
        for c in self.superview!.constraints {
            if let second = c.secondItem as? LoadingButton
...
extension LoadingButton:CAAnimationDelegate {
...
extension LoadingButton: StateLayerDelegate {
...
extension LoadingButton: UIViewControllerTransitioningDelegate{
