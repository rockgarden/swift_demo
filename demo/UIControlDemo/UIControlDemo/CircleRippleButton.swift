...
open class CircleRippleButton: UIButton {
    ...
    open func startAction() {
        if !actionInProgress {
            ...
            originalTitleColor = self.titleColor(for: UIControlState())!
            setTitleColor(UIColor.clear, for: UIControlState())
    ...
    fileprivate func resetToOriginalStatus() {
        ...
        setTitleColor(originalTitleColor, for: UIControlState())
    }
    ...
    fileprivate func pathFor(_ layer : CALayer, radius : CGFloat) -> CGPath {
        ...
        return UIBezierPath(arcCenter: center, radius: radius, startAngle: 0.0, endAngle: CGFloat(2 * Double.pi), clockwise: false).cgPath
    }
