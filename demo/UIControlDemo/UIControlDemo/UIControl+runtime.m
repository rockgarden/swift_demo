...
+ (void)load{
    ...
    dispatch_once(&onceToken, ^{
        ...
        Method mA = class_getInstanceMethod(self, selA);
        Method mB = class_getInstanceMethod(self, selB);

        /// 将methodB的实现添加到系统方法中也就是说将methodA方法指针添加成方法methodB的返回值表示是否添加成功
        BOOL isAdd = class_addMethod(self, selA, method_getImplementation(mB), method_getTypeEncoding(mB));

        /// 添加成功了说明本类中不存在methodB所以此时必须将方法b的实现指针换成方法A的，否则b方法将没有实现。
        if (isAdd) {
            class_replaceMethod(self, selB, method_getImplementation(mA), method_getTypeEncoding(mA));
        } else {
