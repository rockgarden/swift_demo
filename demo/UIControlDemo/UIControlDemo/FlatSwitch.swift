...
/**
 programmaticSwitch = AIFlatSwitch(frame: CGRect(x: 0, y: 0, width: containerView.frame.width, height: containerView.frame.height))
 programmaticSwitch.lineWidth = 2.0
 programmaticSwitch.strokeColor = UIColor.blue
 programmaticSwitch.trailStrokeColor = UIColor.red.withAlphaComponent(0.2)
 programmaticSwitch.autoresizingMask = [.flexibleWidth, .flexibleHeight]
 containerView.addSubview(programmaticSwitch)

 programmaticSwitch.setSelected(!programmaticSwitch.isSelected, animated: true)
 */

import UIKit

/// A flat design switch alternative to UISwitch
@IBDesignable open class FlatSwitch: UIControl {
    ...
    fileprivate func configure() {
        ...
        self.addTarget(self, action: #selector(FlatSwitch.handleTouchUpInside), for: UIControlEvents.touchUpInside)
    }
